<?php
	...
	try {
		$sql = "SELECT species_id, common_name, scientific_name, description, image_url,
		               sunlight_level, ideal_moisture_min, created_at
		        FROM plant_species
		        ORDER BY species_id ASC";
		$stmt = $conn->prepare($sql);
		$stmt->execute();
		$speciesRows = $stmt->fetchAll(PDO::FETCH_ASSOC);
		foreach ($speciesRows as $index => $species) {
			$speciesName = strtolower($species['common_name']);
			$status = in_array($speciesName, ['aloe vera', 'rose'], true) ? 'needs_water' : 'healthy';

			$plants[] = [
				'plant_id' => (int)$species['species_id'],
				'nickname' => $species['common_name'],
				'location' => $index < 3 ? 'Indoor - Living Room Corner' : 'Garden Bed',
				'status' => $status,
				...
				'sunlight_level' => $species['sunlight_level'],
				'ideal_moisture_min' => $species['ideal_moisture_min'],
			];
		}
	} catch (PDOException $e) {
		$errorMsg = "Could not load plants right now.";
	}
		...
		<main>
			...
			<?php elseif ($totalPlants === 0): ?>
				<div class="empty-card">
					...
					<h2>No plants yet</h2>
					<p>Add plants to your database and they will appear here.</p>
				</div>
			<?php else: ?>
				<?php
